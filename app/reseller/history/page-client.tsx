...
import { ArrowLeft, History, Package, DollarSign, Users, Filter, ExternalLink, Download, TrendingUp } from 'lucide-react';
...
type HistoryData = {
  ...
  stats: {
    ...
    networkLabelCount: number;
    totalCreditsAssigned: number;
    networkCreditBalance: number;
    networkSize: number;
  };
};
...
interface ResellerHistoryClientProps {
  historyData: HistoryData;
  session: Session;
}

export default function ResellerHistoryClient({ historyData, session }: ResellerHistoryClientProps) {
  ...
  const filteredTransactions = historyData.transactions.filter(transaction => {
    ...
    if (userFilter !== 'all' && transaction.user.id !== userFilter) {
      return false;
    }
  ...
  // Filter shipments
  const filteredShipments = historyData.recentLabels.filter(shipment => {
    if (userFilter !== 'all' && shipment.user.id !== userFilter) {
      return false;
    }
    return true;
  });
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-4">
          ...
          <h1 className="text-2xl font-bold">Network History</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-green-600">{formatCurrency(historyData.stats.totalNetworkSpent)}</div>
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Network Labels</CardTitle>
            <Package className="h-4 w-4 text-muted-foreground" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{historyData.stats.networkLabelCount}</div>
            <p className="text-xs text-muted-foreground">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-blue-600">{formatCurrency(historyData.stats.totalCreditsAssigned)}</div>
            <p className="text-xs text-muted-foreground">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Network Balance</CardTitle>
            <DollarSign className="h-4 w-4 text-muted-foreground" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold text-green-600">{formatCurrency(historyData.stats.networkCreditBalance)}</div>
            <p className="text-xs text-muted-foreground">
              Available network credits
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{historyData.stats.networkSize}</div>
            <p className="text-xs text-muted-foreground">
              You + your users
            </p>
          </CardContent>
      ...
      <Tabs defaultValue="transactions" className="space-y-4">
        <TabsList className="grid w-full grid-cols-2">
          <TabsTrigger value="transactions">Network Transactions</TabsTrigger>
          <TabsTrigger value="labels">Network Shipping Labels</TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="transactions" className="space-y-4">
                ...
                <div>
                  ...
                  <Select value={userFilter} onValueChange={setUserFilter}>
                    ...
                    <SelectContent>
                      ...
                      {historyData.networkUsers.map((user) => (
          ...
          <Card>
            <CardHeader>
              <CardTitle>Network Transaction History</CardTitle>
              <CardDescription>
                View all transactions across your reseller network
              </CardDescription>
            </CardHeader>
            <CardContent>
              <DataTable
        ...
        <TabsContent value="labels" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Shipping labels created across your reseller network
              </CardDescription>
            </CardHeader>
            <CardContent>
              <DataTable
